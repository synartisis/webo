...
const { readFileAsync, calcHash } = require('../utils')
...
module.exports.resolveDeps = async function resolveDeps(content, options) {
  ...
  await Promise.all(
    elements.map(async el => {
      ...
      if (!type) type = ext
      const dep = createDep(filename, rel, type, filename)
...
async function applyLayout(content, layout, filename, hash) {
...
function htmlDeps(document, includeHtmlLinks = false) {
  ...
  ].filter(el => el.href ? !el.href.startsWith('http:') && !el.href.startsWith('https:') : !el.src.startsWith('http:') && !el.src.startsWith('https:'))
