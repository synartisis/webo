...
module.exports = async function server(entries, WS_PORT) {
  ...
  http.createServer(async (req, res) => {
    ...
    if (!content && (req.url.startsWith('/webo/') || req.url === '/')) return weboServer(req, res, entries, WS_PORT, state.config.srcRoot)
