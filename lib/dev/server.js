...
module.exports = async function server(WS_PORT) {
  ...
  server.on('error', error => {
    if (error.code === 'EADDRINUSE') {
      ...
      server.listen(port, () => utils.log('_GREEN_webo static server listening at ' + port))
    } else {
      throw error
    }
  })
  if (state.entryType !== 'express') server.listen(state.config.dev.port, () => utils.log('_GREEN_webo static server listening at ' + state.config.dev.port))
}
