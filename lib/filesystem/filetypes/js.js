...
module.exports = () => {
  return Object.assign({}, file, {
    ...
    async load() {
      // console.log('JS_LOADER ' + this.path, __dirname, require.resolve('vue'))
      ...
      let bundle = await rollup.rollup({ 
        entry: this.path, 
        plugins: [ resolve(), commonjs(), replace({ 'process.env.NODE_ENV': JSON.stringify('WEBO_development') })],//, nodeGlobals() ],
      ...
      const deps = bundle.modules
        .map(m => relPath(m.id))
        .filter(path => !path.includes('commonjsHelpers') && !path.includes('process-es6/browser') && !path.includes('rollup-plugin'))
