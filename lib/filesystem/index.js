...
async function loadDirectory(root) {
  ...
  if (added) {
    ...
    htmlFiles.forEach(html => html.deps.filter(o => o.url.endsWith('.js')).forEach(dep => dep.devo === 'module' ? files.bundleRoots.push(dep.path) : files.attach(dep.path).load(Object.assign({ raw: true }, { config }))))
    await Promise.all(files.bundleRoots.map(path => files.attach(path).load({ config })))
    if (config.vendorFilename) {
      htmlFiles.forEach(html => {
        const depModules = html.deps.filter(o => o.url.endsWith('.js') && o.devo === 'module').map(o => files.getFile(o.path))
