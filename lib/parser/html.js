...
module.exports = async function(content, options) {
  ...
  const { mode, filename, transpile, bundle, layout, hash } = options
  

  if (layout && !content.includes('<html ')) content = await applyLayout(content, layout, filename)
  ...
  if (transpile) {
    const babelPolyfill = document.createElement('script')
    babelPolyfill.src = '/babel-polyfill.js'
    document.head.appendChild(babelPolyfill)
  }
