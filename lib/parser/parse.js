...
module.exports = async function parse(filename, mode, type) {
  ...
  let deps = []
  if (content !== null) {
    let result = await parsers[ext](content, { mode, filename, type, layout: state.config.layout, transpile: state.proc.transpile, bundle: state.proc.bundle, hash: state.proc.hash })
