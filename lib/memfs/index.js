...
module.exports.attachDirectory = async path => {
  ...
  if (added) {
    ...
    const paths = await async(glob)(path + '/**/*', { nodir: true, ignore: config.exclude })
...
module.exports.touch = async path => {
  ...
  if (file.ext === 'html') {  // check dependencies for type change
    file.deps.forEach(dep => {
      const depFile = getFile(dep.path)
      if (depFile && depFile.type !== dep.type) loadFileContent(dep.path)
    })
  }
...
function calcFileType(path) {
  if (path.endsWith('.html')) return 'html'
  const asHtmlDep = htmlDeps().find(dep => dep.path === path)
  return asHtmlDep ? asHtmlDep.type : null
}

function htmlDeps() {
  return files.filter(o => o.ext === 'html').map(o => o.deps).reduce((flat, deps) => flat.push(... deps) && flat, [])
}
