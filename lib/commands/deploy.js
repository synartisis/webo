...
export default async function deploy(config) {
  ...
  if (spawnRSync.exitCode !== 0) return spawnRSync
...
async function spawnAsync(command, origin) {
  ...
  return new Promise((resolve, reject) => {
    ...
    sp.stderr.on('data', (/** @type {Buffer} */data) => data.toString().split('\n').forEach(line => line ? log(`_LIGHTRED_${line}`) : ''))
    sp.on('close', code => { code === 0 ? resolve({ exitCode: 0, message: 'done' }) : reject({ exitCode: code, message: 'failed' }) })
  })
}
