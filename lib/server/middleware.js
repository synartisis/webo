...
function devoRoutes(req, res, next) {
  if (req.url === '/devo/api') return res.send({ context, roots: filesystem.roots, fileCount: filesystem.files.length, config, bundleRoots: filesystem.bundleRoots, files: filesystem.files })
  if (req.url === '/devo/api/files') return res.send(filesystem.files)
  if (req.url === '/devo/build') return res.send(build(filesystem.roots, filesystem.files))
  req.url = req.url.replace('/devo', '/')
  return serveStatic(__dirname + '/../client/')(req, res, next) 
}

function devoStatic(root) {
  filesystem.loadDirectory(root)
  return (req, res, next) => {
    if (req.url.startsWith('/devo')) return devoRoutes(req, res, next)
    const content = filesystem.getFileContent(root + req.url)
    if (content) {
      res.setHeader('X-Powered-By', 'devo')
      res.send(content)
    } else {
...
function initialize() {
  ...
  expressModule.exports.static = devoStatic
...
function merge(dest = {}, src) {
  for (const prop in src) {
    if (!dest.hasOwnProperty(prop)) { dest[prop] = src[prop]; continue; }
