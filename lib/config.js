...
function getUserOptions() {
  ...
  if (!command) return logAndExit(`Usage: webo command [entry] [flags]\n  Commands: dev, build, deploy, config\n  Flags   : ${Object.keys(options)}`, 1)
  if (command !== 'dev' && command !== 'build' && command !== 'deploy' && command !== 'config') return logAndExit(`Unknown command '${command}'\nCommands allowed : dev, build, deploy, config`, 1)
...
function validateUserArgs(command, userEntry, options) {
  ...
  if (command !== 'dev' && command !== 'build' && command !== 'deploy' && command !== 'config') return `Unknown command '${command}'\nCommands allowed : dev, build, deploy, config`
