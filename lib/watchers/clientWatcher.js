...
module.exports.watchDirectories = paths => {
  paths = paths.filter(path => !watchingDirs.includes(path))
  if (!paths.length) return
  watchingDirs.push(...paths)
  const watcher = chokidar.watch(paths, { ignoreInitial: true, cwd: '.' })
  log('VERBOSE', 'WATCHING CLIENT', paths.join(', '))
...
wsServer.on('connection', ws => {
  ...
  const key = htmlFilePath === '/webo/' ? '/webo/' : ws.upgradeReq.headers['sec-websocket-key']
  sockets.set(key, ws)
})

function notifyClients(message) {
  sockets.forEach((socket, key) => socket.readyState === 1 ? socket.send(JSON.stringify(message)) : sockets.delete(key))
}
