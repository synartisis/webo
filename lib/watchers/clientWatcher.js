...
async function loadDependants(path) {
  files.dependants(path).filter(o => o.ext !== 'html').forEach(async dependant => {
    ...
    files.htmlDependants(dependant.path).forEach(htmlFile => {
      ...
      if (htmlFileDep)
        notifyClient(htmlFile.path, { action: 'inline', url: htmlFileDep.url })
      else
        notifyClient(htmlFile.path, { action: 'refresh' })
    })
  })
}
...
function watchDirectory(root, allFiles, config) {
  ...
  watcher.on('add', async path => {
    ...
    if (!existingFile) Object.assign(files.attach(path), { type })
