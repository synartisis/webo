...
function watchDirectory(root, allFiles, config) {
  ...
  watcher.on('add', async path => {
    ...
    if (path.endsWith('.js') || path.endsWith('.css')) {
      ...
      if (!htmlDep) return
      type = htmlDep.type || (path.endsWith('.js') ? 'script' : 'stylesheet')
    }
    let file = (files.getFile(path) || Object.assign(files.attach(path), { type }))
    loadDependants(path)
  })
  ...
  watcher.on('change', async path => {
    ...
    if (path.endsWith('.html')) {
      ...
      modules.map(o => o.path).forEach(o => !files.htmlDeps('module').includes(o) ? files.attach(o).load() && loadDependants(o) : null)
