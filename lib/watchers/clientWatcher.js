...
function watchDirectory(path) {
  const watcher = chokidar.watch(path, { ignoreInitial: true, cwd: '.' })
  ...
  watcher.on('change', async path => {
    ...
    const filesChanged = await memfs.fileChanged(path)
    if (path.endsWith('.html')) {
      notifyClients({ action: 'refresh' })
    } else {
      filesChanged.forEach(url => notifyClients({ action: 'inline', url }))
    }
  })
