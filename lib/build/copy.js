...
function copy(config, context, roots, files) {
  ...
  copyPolyfill(roots, srcRoot, dest)
...
function copyServerModules(srcRoot, dest) {
  const modules = Object.keys(require.cache).filter(o => !o.includes('node_modules')).map(relPath).filter(o => !o.includes('/webo/') && o !== 'webo.config.js')
...
function copyPolyfill(roots, srcRoot, dest) {
  const polyfillPath = roots.length ? roots[0].replace(srcRoot, dest) + '/polyfill.js' : null
  if (!polyfillPath) return
  fs.writeFileSync(polyfillPath, babelPolyfill)
}
