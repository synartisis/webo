...
const babelPolyfill = fs.readFileSync(__dirname + '/../../node_modules/babel-polyfill/dist/polyfill.min.js', 'utf8')

function removeDevoArtifacts(document) {
  const devoScript = document.querySelector('script[src="/devo/devo.js"]')
  if (devoScript) devoScript.parentNode.removeChild(devoScript)
  ;[... document.querySelectorAll('[devo]')].forEach(el => el.removeAttribute('devo'))
}
...
function cacheBusting(document, files, deps) {
  deps.forEach(dep => {
    ...
    if (dep.url.endsWith('.js')) hashElement(depFile, document, 'script', 'src', dep.url)
    if (dep.url.endsWith('.css')) hashElement(depFile, document, 'link', 'href', dep.url)
    if (dep.url.endsWith('.png')) hashElement(depFile, document, 'img', 'src', dep.url)
  })
}
...
function injectVendorScript(document, hash) {
  // const firstDevoScript = document.querySelector('script[devo]')
  // if (!firstDevoScript) return false
  ...
  // firstDevoScript.parentNode.insertBefore(vendorScript, firstDevoScript)
  return true
}
