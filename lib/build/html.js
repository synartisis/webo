...
function removeWeboArtifacts(document) {
  ...
  ;[... document.querySelectorAll('[webo-type]')].forEach(el => el.removeAttribute('webo-type'))
}
...
function injectBabelPolyfill(document, file, files, polyfillPath) {
...
module.exports = (files, roots) => {
  const polyfillPath = roots.length ? '/polyfill.js' : null
  files.filter(file => file.ext === 'html').forEach(file => {
    if (file.exclude) return
    const dom = new JSDOM(file.built.content)
    ...
    // }
    removeWeboArtifacts(document)
    injectBabelPolyfill(document, file, files, polyfillPath)
    cacheBusting(document, files, file.deps)
    file.built.content = dom.serialize()
  })
}