...
async function init(mode, root, options) {
  ...
  // await Promise.all(state.entries.map(entry => updateEntry(entry, true)))
...
//       const { deps } = await parsers(entry)
//       deps.forEach(attachDep)
//       // state.deps.push(... deps)
...
async function loadFile(filename) {
  ...
  await Promise.all(
    deps.filter(dep => !state.deps.find(o => o.path === dep.path)).map(async dep => {
      state.deps.push(dep)
