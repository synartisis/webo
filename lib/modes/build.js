...
module.exports = async function build({ type, entry, srcRoot, staticRoots, serverRoot, serve, parserOptions }) {
    ...
    await Promise.all(
      files.filter(o => o.content).map(file => {
        ...
        return file.deps.map(async dep => {
          ...
          if (dep.ref) file.content = file.content.replace(new RegExp(dep.ref, 'g'), hashFilename(dep.ref, depFile.hash))
        })
      })
    )
  }
  ...
  await Promise.all(
    extraStaticFiles.map(filename => {
      ...
      logv('_GRAY_extra file copied:', filename)
  ...
  const serveServerPaths = serve ? typeof serve === 'string' ? [serve] : serve : null
  if (serveServerPaths) {
    await Promise.all(
      serveServerPaths.map(async serverPath => {
        const serverPathFiles = await globAsync(serverPath + '/**', { nodir: true })
        return await Promise.all(
          serverPathFiles.map(filename => {
            let destFilename = filename.replace(path.dirname(serverPath) + '/', distRoot)
            return copyFile(filename, destFilename, null, distFiles, copied)
          })
        )
      })
  ...
  log(`${counters.changed} _GREEN_files copied`)
...
async function copyFile(srcFilename, destFilename, content, distFiles, copied) {
  ...
  if (srcHash !== distHash) {
    ...
    counters.changed++
    log('_GRAY_' + destFilename.replace(distRoot, ''))
    mkdirp(destDirname)
    if (content) {
