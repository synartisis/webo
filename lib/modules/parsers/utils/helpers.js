const { relPath, readFileAsync, calcAssetPath, rewritePath, log, logv, calcHash } = require('../../../utils/utils.js')


function logParserError(label, file, error) {
  log(`_LIGHTRED_Parse error: ${file.filename} [${file.type}] _GRAY_[referrer:${file.referrer}] ${label} ${error.toString()}`)
  logv(`file with error: ${JSON.stringify(file, null, 2)}`)
...
const babelPresets = {
  ...
  minify: [[ require.resolve('babel-preset-minify'), { evaluate: false, mangle: false, builtIns: false }]]
}
