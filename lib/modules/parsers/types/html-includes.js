...
module.exports.applyIncludes = async function applyIncludes(filename, content, srcRoot) {
  ...
  const { includesContent, includesDeps } = await applyPartials(filename, result.content)
  if (result.layoutPath) includesDeps.push(result.layoutPath)
  Object.assign(result, { content: includesContent, includesDeps })
  return result
}
...
async function applyPartials(filename, content) {
  let includesContent = content
  const includesDeps = []
  const partials = String(includesContent).match(/(\[\[.*import.*\]\])/ig)
  // console.log(filename, '===')
  if (!partials || !partials.length) return { includesContent, includesDeps }
  await Promise.all(
    partials.map(async partial => {
      ...
      includesDeps.push(path.posix.normalize(partialFilename))
      // console.log('**', partialFilename, filename, relPath , path.posix.normalize(partialFilename))
      ...
      partialDeps.forEach(el => {
        let attr = 'href' in el ? 'href' : 'src'
        el[attr] = relPath + ( relPath && !relPath.endsWith('/') && !el[attr].startsWith('/') ? '/' : '' ) + el[attr].replace('about:blank', '')
      })
      includesContent = includesContent.replace(partial, domPartial.serialize())
    })
  )
  return { includesContent, includesDeps }
}
