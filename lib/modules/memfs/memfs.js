...
async function init({ mode, srcRoot, staticRoots, serverRoot, parserOptions, verbose, watch, webSocketPort }) {
  ...
  const entries = await globAsync(path.posix.join(srcRoot, `**/*\.@(html|vue)`), { cwd: '.', nodir: true, ignore: ['**/_*'] })
  // console.log(entries)
...
async function touchFile(event, filename) {
  ...
  const dependants = files.filter(file => file.deps && file.deps.some(o => o.filename === filename))
